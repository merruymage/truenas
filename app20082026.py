...
import math
import sqlite3 
import socket # <--- NUEVO: Para obtener el nombre del equipo en la red
...
# Ajuste Senior para que funcione perfecto como .exe o como .py
if getattr(sys, 'frozen', False):
    # Si está corriendo como un ejecutable compilado (.exe)
...
ARCHIVO_BD = os.path.join(DIRECTORIO_BASE, 'sesion_guardada.db')
...
class AppAuditoriaAvanzada:
    def __init__(self, root):
        self.root = root
        self.root.title("Auditoría NAS & Drive - Panel Gerencial Colaborativo (Ing. Mariana)")
        ...
        # Datos del equipo para auditoría forense
    ...
    # ------------------------------------------
    # MÓDULO DE BASE DE DATOS (ANTIBLOQUEO WAL)
    # ------------------------------------------
    def inicializar_bd(self):
        try:
            # timeout=15 hace que espere si otro usuario tiene la BD bloqueada
            conn = sqlite3.connect(self.archivo_bd, timeout=15.0)
            cursor = conn.cursor()
            
            # Activar el modo WAL para permitir lectura y escritura simultánea
    ...
    def abrir_ventana_ajustes(self):
        ...
        f_salida = ttk.LabelFrame(v_ajustes, text=" 📂 Rutas de Base de Datos y Reportes (Colaboración) ")
        f_salida.pack(fill="x", padx=10, pady=5)

        ttk.Label(f_salida, text="Ruta de Base de Datos (SQLite .db):").pack(anchor="w", padx=5)
        ...
        def buscar_bd():
            archivo = filedialog.asksaveasfilename(title="Seleccionar o Crear Base de Datos", defaultextension=".db", filetypes=[("SQLite DB", "*.db"), ("Todos", "*.*")])
            if archivo:
                ent_bd.delete(0, tk.END)
                ent_bd.insert(0, archivo)
        ttk.Button(f_bd, text="📁 Buscar", command=buscar_bd).pack(side="left", padx=5)

        ttk.Label(f_salida, text="Ruta del Reporte Excel (.csv):").pack(anchor="w", padx=5, pady=(5,0))
        ...
        def buscar_rep():
            archivo = filedialog.asksaveasfilename(title="Guardar Reporte Como", defaultextension=".csv", filetypes=[("CSV Excel", "*.csv"), ("Todos", "*.*")])
            if archivo:
                ent_rep.delete(0, tk.END)
                ent_rep.insert(0, archivo)
